...
class xTransv0_parallel(nn.Module):

    def __init__(self, configs):
        super(xTransv0_parallel, self).__init__()
        ...
        self.dec = nn.Linear(2*self.patch_num*self.d_model, self.pred_len)
    ...
    def forward(self, x_batch, x_batch_mark, y_batch_zero, y_batch_mark):
        ...
        # Encoder
        enc_t = self.t_embedding(enc_in)  # [B, N, Pnum, dmodel]
        enc_t = torch.reshape(enc_t, (enc_t.shape[0]*enc_t.shape[1],enc_t.shape[-2],enc_t.shape[-1]))  # [B*N, Pnum, d_model]
        # print("Xt shape: ", enc_t.shape)
        enc_t, _ = self.encoder_t(enc_t, x_mark_enc)  # [B*N, Pnum, d_model]
        enc_t = torch.reshape(enc_t, (-1, self.num_nodes, enc_t.shape[-2],enc_t.shape[-1]))

        enc_n = self.s_embedding(enc_in).permute(0, 2, 1, 3)
        enc_n = torch.reshape(enc_n, (enc_n.shape[0]*enc_n.shape[1],enc_n.shape[-2],enc_n.shape[-1]))  # [B*Pnum, N, d_model]
        # print("Xn shape: ", enc_n.shape)
        enc_n, _ = self.encoder_n(enc_n, x_mark_enc)   # [B*Pnum, N, d_model]
        enc_n = torch.reshape(enc_n, (-1, self.patch_num, enc_n.shape[-2],enc_n.shape[-1]))
        enc_n = enc_n.permute(0, 2, 1, 3)
        
        enc_out = torch.cat([enc_t, enc_n], -1)
