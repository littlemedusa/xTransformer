...
class xTransformer_v1(nn.Module):

    def __init__(self, configs):
        super(xTransformer_v1, self).__init__()
        ...
        self.residual_t = nn.Linear(configs.d_model, configs.d_model)
        ...
        self.residual_n= nn.Linear(configs.d_model, configs.d_model)
        self.residual_io = nn.Linear(configs.d_model, configs.d_model)
    ...
    def forward(self, x_batch, x_batch_mark, y_batch_zero, y_batch_mark):
        ...
        x_enc = self.enc_embedding(x_enc)  # [B, N, Pnum, dmodel]
        enc_in = x_enc
        
        # Encoder
        enc_t = torch.reshape(enc_in, (enc_in.shape[0]*enc_in.shape[1],enc_in.shape[-2],enc_in.shape[-1])) 
        ...
        enc_t = self.residual_t(enc_t)
        enc_t = enc_t + enc_in

        enc_n = enc_t.permute(0, 2, 1, 3)
        ...
        enc_n = self.residual_n(enc_n)
        enc_n = enc_n + enc_t
        
        enc_out = self.residual_io(enc_n)
