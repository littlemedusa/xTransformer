...
class xTransv0_biembed_gate(nn.Module):

    def __init__(self, configs):
        super(xTransv0_biembed_gate, self).__init__()
        ...
        self.t_embedding = Temporal_Embedding(self.num_nodes, self.patch_num, self.patch_len, self.d_model, configs.dropout, configs.embedding_range)
        self.s_embedding = Spatial_Embedding(self.num_nodes, self.patch_num, self.patch_len, self.d_model, configs.dropout, configs.embedding_range)
        ...
        self.n_gate_weights = nn.Linear(self.d_model, self.d_model)
    ...
    def forward(self, x_batch, x_batch_mark, y_batch_zero, y_batch_mark):
        ...
        enc_n_weights = self.sigmoid(self.n_gate_weights(enc_n))
        # print("X shape: ", enc_n.shape)
        enc_n, _ = self.encoder_n(enc_n)   # [B*Pnum, N, d_model]
        enc_n = enc_n * enc_n_weights  # (element-wise multiplication)
...
class Temporal_Embedding(nn.Module):
    def __init__(self, num_nodes, patch_num, patch_len, d_model, dropout=0.1, embedding_range=0.02):
        super(Temporal_Embedding, self).__init__()
        self.patch_embedding = nn.Parameter(
                nn.init.uniform_(torch.empty(patch_num, d_model), -embedding_range, embedding_range),
                requires_grad=True
            )
        self.dropout = nn.Dropout(dropout)
...
class Spatial_Embedding(nn.Module):
    def __init__(self, num_nodes, patch_num, patch_len, d_model, dropout=0.1, embedding_range=0.02):
        ...
        self.node_embedding = nn.Parameter(
                nn.init.uniform_(torch.empty(num_nodes, d_model), -embedding_range, embedding_range),
                requires_grad=True
            )
        self.dropout = nn.Dropout(dropout)
