...
class WeightedMSELoss:
    ...
    def __call__(self, y_pred, y_true):
        return weightedMSEloss(y_pred, y_true)    
    
    
def weightedMSEloss(y_pred, y_true):
    # print(y_pred.shape, y_true.shape)
    """
    y_pred: [B, T, N, 2] -> 2 = mean + std 
    y_true: [B, T, N]
    """
    expectation, variance = y_pred[..., 0], y_pred[..., 1]
    # print(expectation.shape, variance.shape)
    squared_diff = ((y_true - expectation) ** 2) * (variance ** 2 + 1)
...
def choose_model(data, data_name):
    model_select_args = dict()
    
    # fixed
    if data_name in ("ETTH1", "ETTH2", "ETTM1", "ETTM2", "RATE"):
        model_select_args["axis"] = 't'
        model_select_args["compress"] = 0
    if data_name in ("ECL", "TRAFFIC", "WEATHER", "SOLAR"):
        model_select_args["axis"] = 'n'
        model_select_args["compress"] = 1
        

    return model_select_args
